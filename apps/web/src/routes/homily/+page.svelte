<script lang="ts">
  /** Homily tips for a chosen day — a ministry-mode surface. */
  ...
  const homily = $derived(((day?.sections ?? []) as Section[]).find((s) => s.key === "homily") ?? null);
  const gospel = $derived(((day?.sections ?? []) as Section[]).find((s) => s.key === "gospel") ?? null);
</script>

<svelte:head>
  <title>Homily Tips — God's Word</title>
  <meta name="description" content="Preaching notes on the Gospel of the day." />
</svelte:head>
...
<div class="page-body">
  ...
  {:else}
    <p class="empty">
      <span class="e-ic">{@html icons.quote}</span>
      No homily tip is recorded for this day.
    </p>
  {/if}
</div>
